...
# Define log and error files inside a logs subdirectory within the output directory
LOG_DIR="./logs"
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/picrust2_${Th}.log"
ERR_FILE="${LOG_DIR}/picrust2_${Th}.err"

# Run PICRUSt2 pipeline
picrust2_pipeline.py \
	-s "${FASTA_FILE}" -i "${ASV_TABLE}" -o "${OUTPUT_DIR}" \
	-p "${CPU}" \
