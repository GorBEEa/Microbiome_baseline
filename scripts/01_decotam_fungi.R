...
# Load data
count_tab <- read.table("data/dada2/2023_fungi_GorBEEa_prj_ASVs_counts.tsv", header=T, row.names=1,
                        check.names=F, sep="\t")

tax_tab <- as.matrix(read.table("data/dada2/2023_fungi_GorBEEa_prj_ASVs_taxonomy.tsv", header=T,
                                row.names=1, check.names=F, sep="\t"))

sample_info_tab <- read.delim("data/dada2/fungi_sample_info.tsv", header=T, row.names=1, check.names=F, sep="\t")
...
ps.gbp23.f <- prune_samples(sample_data(ps.nocont)$type == "sample", ps.nocont)
...
table(sample_data(ps.gbp23.f)$type)

# Save the ps.gbp23 object to an RDS file
saveRDS(ps.gbp23.f, file = "data/ps.gbp23.f.RDS")
